...
void process_output()
{
    ...
    // check format is "TCPS[Port] , TCPC[IP],[Port]"

    if (outstream_type != "TCPS" && outstream_type != "TCPC")
    {
...
void process_input()
{
    ...
    if (instream_type != "TCPS" && instream_type != "TCPC")
    {
...
{
    int opt;
    while ((opt = getopt(argc, (char *const *)argv, "o:i:e:b:")) != -1)
    {
        switch (opt)
        {
        case 'o':
